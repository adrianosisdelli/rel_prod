...
Route::post('/report/01', function() {
	...
	$sql = "select g.Descricao grupo , u.Login usuario , f.cpfcnpj 'cpfcnpj' , c.Numero_Contrato contrato , h.Dt_Ocorr 'dt_historico' , os.Descricao 'ocorrencia' , os.Ocorrencia_Positiva 'ocorrencia_positiva' , os.Descricao_Complemento 'descricao_complemento' , h.Complemento 'complemento_historico' , os.CPC cpc , os.Cod_Ocorr_Sistema 'codigo_ocorrencia' from cob.Historico h left join cob.Contrato c on h.Id_Contrato = c.Id_Agrupamento left join par.Grupo g on c.Id_Grupo = g.Id_Grupo left join usu.Usuario u on h.Id_Usuario_Inseriu = u.Id_Usuario left join cob.Financiado f on c.Id_Financiado = f.id_financiado left join par.Ocorrencia_Sistema os on h.Id_Ocorrencia_Sistema = os.Id_Ocorrencia_Sistema where 1 = 1 and c.Id_Grupo in([carteira]) and h.Dt_Ocorr between '[data_inicial] 00:00:00.000' and '[data_final] 23:59:59.000' and h.Id_Usuario_Inseriu not in (1, 170, 61) and os.Cod_Ocorr_Sistema not in ('00008', '00007', '00002') ";
